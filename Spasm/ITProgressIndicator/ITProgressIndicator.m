...
- (id)init {
  if ((self = [super initWithFrame:NSMakeRect(0, 0, 60, 60)]) == nil) {
    return nil;
  }
...
- (void)initLayers {
  self.lengthOfLine = 12;
  self.widthOfLine = 2;
  self.numberOfLines = 9;
  self.innerMargin = 8;
  self.animationDuration = 1;
  self.color = [NSColor colorWithWhite:0.6 alpha:1];
    ...
    [progressImage lockFocus];
    {
        [NSGraphicsContext saveGraphicsState];
        {
          ...
          /*NSBezierPath *shadow = [NSBezierPath bezierPathWithRoundedRect:
                                  NSMakeRect((NSWidth(r) / 2) - (self.widthOfLine / 2 + 1),
                                             (NSHeight(r) / 2) - self.innerMargin - self.lengthOfLine - 1,
                                             self.widthOfLine + 2, self.lengthOfLine + 2)
                                                                 xRadius:self.widthOfLine / 2 + 1
                                                                 yRadius:self.widthOfLine / 2 + 1];*/
            ...
            ^(NSUInteger lineNumber) {
              ...
              /*NSBezierPath *shadowInstance = [shadow copy];
              shadowInstance = [shadowInstance rotatedBezierPath:((2 * M_PI) / self.numberOfLines * lineNumber) + M_PI
                                                  aboutPoint:NSMakePoint(NSWidth(r) / 2, NSHeight(r) / 2)];
